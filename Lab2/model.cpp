...
public:
    ...
    CalcNode() : x(0.0), y(0.0), z(0.0), pressure(0.0), wx(0.0), wy(0.0), wz(0.0), vx(0.0), vy(0.0), vz(0.0)
    {
    }

    // Конструктор с указанием всех параметров
    CalcNode(double x, double y, double z, double pressure, double wx, double wy, double wz, double vx, double vy, double vz)
            : x(x), y(y), z(z), pressure(pressure), wx(wx), wy(wy), wz(wz), vx(vx), vy(vy), vz(vz)
    {
    }
    ...
    void move(double tau) {
        ...
        x = -(y+24.5) * wz * tau + x * sqrt(1-pow(wz*tau,2));
        y = -24.5+x * wz * tau + (y+24.5) * sqrt(1-pow(wz*tau,2));
    }
};
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double vz = 10;
            double vx = wz*pointX;
            double vy = wz*(pointY+24.5);
            double pressure = pointZ * 10 +10*sqrt((pointY+24.5)*(pointY+24.5)+pointX*pointX);
            nodes[i] = CalcNode(pointX, pointY, pointZ, pressure, 0.0, 0.0, wz, vx, vy, vz);
        }
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "ARROW" + std::to_string(snap_number) + ".vtu";
